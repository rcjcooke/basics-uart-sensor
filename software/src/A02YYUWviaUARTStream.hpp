...
private:
  ...
  bool mProcessed;
  // The last time the sensor was asked to update the distance reading / ms since last reset
  unsigned long mLastReadTime = 0;
  // The time the last sensor reading was successful / mm since reset (i.e. a full data packet was received)
  unsigned long mLastReadSuccess = 0;
  // The result of the last read request: (0 if successful, -1 if there's a checksum error, -2 if the frame wasn't read correctly). If there wasn't enough data available, or this was called before the next read interval is due, then this returns 0;
